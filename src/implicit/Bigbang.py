...
def big_bang(indexes, df, nodes, battery_map, dt):
    ...
    materials_number = int(
        len(indexes)
    )  # Amount of different materials present in the test
    ...
    for i in range(materials_number):
        ...
        _type = df._get_value(
            idx, "Type"
        )  # From de data frame (df) takes the str Type at the index i
    ...
    for j in range(materials_number):
        ...
        e_modulus = df.loc[
            _material, "e_modulus"
        ]  # Takes elastic modulus for each material
        ...
        ]  # Takes bulk modulus for each material

        # Material class instantiation
        material = Material(
            density, e_modulus, state, bulk_modulus, thickness, _material
        )  # material instantiation
        materials_summary.append(material)  # stores each material in a list
        materials_thickness.append(
            material.thickness
        )  # stores each material thickness in a list
        materials_e_modulus.append(
            material.e_modulus
        )  # stores each elastic modulus in a list
    ...
    for _dimensionless_length in range(
        len(battery_map)
    ):  # computes the dimensionless thickness
        _id = battery_map[_dimensionless_length]
        dimensionless_thickness = _dict[_id] / length
        material_dimensionless_length.append(
            dimensionless_thickness
        )  # save each dimensionless thickness in a list
    ...
    for j in range(
        len(material_dimensionless_length)
    ):  # checking total dimensionless length = 1
