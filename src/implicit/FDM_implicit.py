...
def fdm_implicit(
    ...
    interphase_node = []
    for _interphase_position in range(
        len(interphase_position)
    ):  # compute an integer value for each interphase
    ...
    for j in range(0, n_steps):  # Implicit Finite Difference Method implementation
        ...
        if j == 0:
            ...
            for node_count in range(0, nodes):
                if (interphase_node[interphase_count] < interphase_node[-1] + 1) and (
                    condition is True
                ):  # Perform at
                    ...
                    if node_count == 1:  # first node
                        ...
                        formulation.time_0_node_1_dirichlet(
                            gamma, u_left, initial_velocity, dt, uj0[node_count]
                        )
                        a[node_count, node_count] = formulation.a_i_i
                    ...
                    if node_count == 2:  # second node
                        ...
                        formulation.time_0_node_2_dirichlet(
                            phi, u_left, initial_velocity, dt, uj0[node_count]
                        )
                    ...
                    if (
                        (node_count > 2)
                        and (node_count < interphase_node[interphase_count] - 1)
                        and (node_count != interphase_node[interphase_count - 1] + 1)
                    ):  # central nodes
                        phi = phi_map[interphase_count]
                        formulation.time_0_internal_node(
                            phi, initial_velocity, dt, uj0[node_count]
                        )
                    ...
                    if (
                        node_count == interphase_node[interphase_count] - 1
                    ):  # node that takes the interphase right
                        gamma = gamma_map[interphase_count]
                        formulation.time_0_node__1_interphase(
                            gamma, initial_velocity, dt, uj0[node_count]
                        )
                        a[node_count, node_count - 1] = formulation.a_i_i_1
                        a[node_count, node_count] = formulation.a_i_i
                        a[node_count, node_count + 1] = formulation.a_i_i1
                    ...
                    ):  # node that takes the interphase left
                        gamma = gamma_map[interphase_count]
                        formulation.time_0_node_1_interphase(
                            gamma, initial_velocity, dt, uj0[node_count]
                        )
                        a[node_count, node_count - 1] = formulation.a_i_i_1
                        a[node_count, node_count] = formulation.a_i_i
                    ...
                    if (
                        node_count == interphase_node[interphase_count] + 1
                    ):  # takes a node at its left interphase
                        gamma = gamma_map[interphase_count]
                        formulation.time_0_node_1_interphase(
                            gamma, initial_velocity, dt, uj0[node_count]
                        )
                    ...
                    if (node_count > interphase_node[-1] + 1) and (
                        node_count < nodes - 2
                    ):  # central nodes
                        phi = phi_map[interphase_count]
                        formulation.time_0_internal_node(
                    ...
                    if node_count == nodes - 2:  # Penultimate node
                        gamma = gamma_map[interphase_count]
                        formulation.time_0_penultimate_node(
                            gamma, initial_velocity, dt, uj0[node_count]
                        )
                ...
                if (interphase_node[interphase_count] < interphase_node[-1] + 1) and (
                    ...
                    if node_count == 0:
                        ...
                        if j > (len(_y) + 200):  # Neumann
                            gamma = gamma_map[interphase_count]
                            formulation.node_1_neumann(
                                gamma, uj0[node_count], uj_1[node_count]
                            )
                            a[node_count, node_count - 1] = formulation.a_i_i_1
                            a[node_count, node_count] = formulation.a_i_i
                            a[node_count, node_count + 1] = formulation.a_i_i1
                    ...
                    if node_count == 1:
                        ...
                        if j < (len(_y) - 1):  # Input
                            u_left: float = _y[j + 1]
                            gamma = gamma_map[interphase_count]
                            formulation.node_1_dirichlet(
                                gamma, uj0[node_count], uj_1[node_count], u_left
                            )
                            a[node_count, node_count] = formulation.a_i_i
                            a[node_count, node_count + 1] = formulation.a_i_i1
                            b[node_count] = formulation.b
                    ...
                    if node_count == 2:
                        ...
                        if j < (len(_y) - 1):  # Input
                            u_left: float = _y[j + 1]
                            phi = phi_map[interphase_count]
                            formulation.node_2_dirichlet(
                                phi, uj0[node_count], uj_1[node_count], u_left
                            )
                            a[node_count, node_count - 1] = formulation.a_i_i_1
                            a[node_count, node_count] = formulation.a_i_i
                            a[node_count, node_count + 1] = formulation.a_i_i1
                        ...
                        if j > (len(_y) + 200):  # Neumann
                            phi = phi_map[interphase_count]
                            formulation.internal_node(
                                phi, uj0[node_count], uj_1[node_count]
                            )
                            a[node_count, node_count - 2] = formulation.a_i_i_2
                            a[node_count, node_count - 1] = formulation.a_i_i_1
                            a[node_count, node_count] = formulation.a_i_i
                    ...
                    if (
                        node_count == interphase_node[interphase_count] - 1
                    ):  # node that takes the interface right
                        gamma = gamma_map[interphase_count]
                        formulation.node__1_interphase(
                            gamma, uj0[node_count], uj_1[node_count]
                        )
                        a[node_count, node_count - 1] = formulation.a_i_i_1
                        a[node_count, node_count] = formulation.a_i_i
