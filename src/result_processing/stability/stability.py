...
def stability_f(dt_array, n_steps, reading_path, save_path, main_path):
    ...
    slice_dict = functions.dict_iter_number_f(
        dt_array, n_steps
    )  # Return a dict to allow the slicing of the dataframes
    ...
    for file in os.listdir():  # for loop for all the files
        if file.endswith(".csv"):  # if they are .csv
            ...
            iter_number = functions.read_iter_number(
                dt, slice_dict
            )  # Checks the iter number to slice the corresponding to the current dataset
            functions.slice_df(
                iter_number, df, nodes_str, dt_str, save_path, reading_path
...
def equal_data_files(n_steps, dt_array, reading_path, save_path):
    ...
    max_time_intersection = max_time_intersection_f(
        n_steps, dt_array
    )  # Function that measure the common maxium time in all the files
    length_dataset = lenght_dataset_f(
        max_time_intersection, dt_array
    )  # Function that measure the amount of data in the biggest file
    ...
    for file in os.listdir():
        if file.endswith(".csv"):
            ...
            dt_str = f"{dt_str[-3]}-{dt_str[-2]}"  # Takes the dt in the file name
            dt = float(dt_str)
            y, lowest_time, highest_time = time_array(
                dt, n_steps, max_time_intersection
            )  # Generates the time array
            function = interpolate.interp1d(
                y, df
            )  # Interpolate function of time vs deformation
            interpolation(
                length_dataset,
                function,
                lowest_time,
                ...
                save_path,
                reading_path,
            )  # interpolates adding the number of data as the biggest file
