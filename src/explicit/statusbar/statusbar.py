...
class status_bar:
    ...
    def update(self, c_step):
        ...
        sys.stdout.write(
            "\r"
            + f"{self.bold}{self.gx_return(perc)} {perc}%{self.endc} | Step: {self.green}{c_step}/{self.steps}{self.endc} | CPU Time:{self.cyan} {np.round(time.time() - self.star_time, 4)}s{self.endc}"
        )
        sys.stdout.flush()
